...
function top_nav_menu() {
    // Pobranie elementów menu o nazwie [translate: "Górne"]
    $menu = wp_get_nav_menu_items('Górne');

    $items_by_id = [];

    // Tworzymy podstawową strukturę dla każdego elementu menu
    foreach ($menu as $item) {
        $items_by_id[$item->ID] = [
            ...
            'children'  => [] // miejsce na zagnieżdżone elementy
        ];
    }

    // Tablica wynikowa – elementy najwyższego poziomu 
    $result = [];

    // Łączenie elementów w jedną strukturę
    foreach ($items_by_id as $id => &$item) {
        // Jeżeli element ma rodzica → dodajemy go do children rodzica
        if ($item['parent_id'] != 0) {
            $items_by_id[$item['parent_id']]['children'][] = &$item;
        } else {
            // Jeżeli nie ma rodzica → element główny menu
            $result[] = &$item;
        }
    }

    // Zwracamy drzewo menu
    return $result;
}

// Rejestracja endpointu REST API dla menu Górne
add_action('rest_api_init', function() {
    register_rest_route(
        ...
        [
            'methods'  => 'GET',
            'callback' => 'top_nav_menu', // funkcja zwracająca dane menu
        ]
    );
});

function main_nav_menu() {
    // Pobranie elementów menu o nazwie [translate: "Główne"]
    $menu = wp_get_nav_menu_items('Główne');
...
// Rejestracja endpointu REST API dla menu Główne
add_action('rest_api_init', function() {
    register_rest_route(
        'wp/v2',
        'glowne-menu',
        [
            'methods'  => 'GET',
            'callback' => 'main_nav_menu', // funkcja zwracająca dane menu
        ]
    );
});

function bottom_nav_menu() {
    // Pobranie elementów menu o nazwie [translate: "Dolne"]
    $menu = wp_get_nav_menu_items('Dolne');
...
// Rejestracja endpointu REST API dla menu Dolne
add_action('rest_api_init', function() {
    register_rest_route(
        'wp/v2',
        'dolne-menu',
        [
            'methods'  => 'GET',
            'callback' => 'bottom_nav_menu', // funkcja zwracająca dane menu
        ]
    );
});
